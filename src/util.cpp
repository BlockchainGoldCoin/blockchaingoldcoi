...
const char * const BITCOIN_CONF_FILENAME = "blockchaingoldcoin.conf";
const char * const BITCOIN_PID_FILENAME = "blockchaingoldcoin.pid";
...
#else
    const char* pszModule = "blockchaingoldcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Blockchaingoldcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Blockchaingoldcoin";
#else
    // Unix
    return pathRet / ".blockchaingoldcoin";
#endif
#endif
}
